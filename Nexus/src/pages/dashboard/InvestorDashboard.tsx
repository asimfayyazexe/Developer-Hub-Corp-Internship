...
import { Users, PieChart, Filter, Search, PlusCircle, CalendarDays, Wallet } from 'lucide-react';
...
import { getConfirmedMeetingsForUser, getWalletBalance } from '../../data/collaborationHub';
        ...
        <CardBody>
          {confirmedMeetings.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {confirmedMeetings.slice(0, 2).map(meeting => {
                const partnerId = meeting.requesterId === user.id ? meeting.recipientId : meeting.requesterId;
                const partner = findUserById(partnerId);

                return (
                  <div key={meeting.id} className="flex items-center rounded-md border border-gray-200 p-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {meeting.date} - {meeting.startTime} - {partner?.name ?? 'Nexus member'}
                      </p>
                    </div>
                  </div>
                );
